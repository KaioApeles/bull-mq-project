...
class Queue extends queue_getters_1.QueueGetters {
    constructor(name, opts, Connection) {
        super(name, Object.assign({ sharedConnection: utils_1.isRedisInstance(opts === null || opts === void 0 ? void 0 : opts.connection) }, opts), Connection);
    ...
    async add(name, data, opts) {
        if (opts && opts.repeat) {
            ...
            const jobId = utils_1.jobIdForGroup(opts, data, { limiter: this.limiter });
    ...
    addBulk(jobs) {
        return job_1.Job.createBulk(this, jobs.map(job => ({
            name: job.name,
            data: job.data,
            opts: Object.assign(Object.assign(Object.assign({}, this.jobsOpts), job.opts), { jobId: utils_1.jobIdForGroup(job.opts, job.data, { limiter: this.limiter }) }),
        })));
    }
    /**
     ...
     */
    drain(delayed = false) {
        return scripts_1.Scripts.drain(this, delayed);
    }
    /**
    ...
    async retryJobs(opts = {}) {
        let cursor = 0;
        do {
            cursor = await scripts_1.Scripts.retryJobs(this, opts.state, opts.count, opts.timestamp);
        } while (cursor);
    }
    /**
